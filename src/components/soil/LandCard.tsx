...
const SOIL_COLORS: Record<string, { from: string; to: string; text: string; light: string }> = {
  Alluvial: { from: 'from-emerald-700', to: 'to-teal-600', text: 'text-emerald-100', light: 'bg-emerald-50' },
  Black:    { from: 'from-gray-800', to: 'to-gray-700', text: 'text-gray-200', light: 'bg-gray-100' },
  Red:      { from: 'from-rose-700', to: 'to-orange-600', text: 'text-rose-100', light: 'bg-rose-50' },
  Laterite: { from: 'from-orange-700', to: 'to-amber-600', text: 'text-orange-100', light: 'bg-orange-50' },
  Sandy:    { from: 'from-amber-600', to: 'to-yellow-500', text: 'text-amber-100', light: 'bg-amber-50' },
  Clay:     { from: 'from-stone-700', to: 'to-stone-600', text: 'text-stone-200', light: 'bg-stone-100' },
  Loamy:    { from: 'from-lime-700', to: 'to-green-600', text: 'text-lime-100', light: 'bg-lime-50' },
};
...
export default function LandCard({ farm, onClick }: { farm: FarmlandCardData; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${soil.from} ${soil.to}`} />

      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="text-[17px] font-black text-gray-900 leading-tight truncate tracking-tight">{farm.landName}</h3>
            {loc && (
          ...
          {hasScore ? (
            <div className={`shrink-0 w-11 h-11 rounded-full border-[3px] border-emerald-50 bg-white flex flex-col items-center justify-center shadow-sm`}>
              <span className="text-[13px] font-black text-emerald-600 leading-none">{farm.report!.score}</span>
            </div>
          ) : (
