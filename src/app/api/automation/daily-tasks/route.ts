...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Format data beautifully for n8n to digest
    const alerts = crops
      // 1. THE FIX: Filter out any orphaned crops where the user no longer exists
      .filter(crop => crop.userId != null) 
      .map(crop => {
        const pendingTasks = crop.tasks.filter((t: any) => !t.isCompleted && new Date(t.scheduledDate) <= todayEnd);
        
        // 2. SAFETY CHECK: Use optional chaining (?.) and fallbacks
        const user = crop.userId as any; 

        return {
          farmerName: user?.name || 'Unknown Farmer',
          phone: user?.phone || 'No Phone',
          language: user?.preferredLanguage || 'en',
          district: crop.location?.district || user?.district || 'Unknown',
          cropName: crop.cropName,
          tasks: pendingTasks.map((t: any) => ({
            id: t._id,
            title: t.title,
