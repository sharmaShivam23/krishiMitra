...
export async function POST(req: Request) {
  try {
    ...
    if (!existingUser) {
      return NextResponse.json({ error: 'No account found with this phone number. Please register.' }, { status: 404 });
    }
    ...
    await twilioClient.messages.create({
      body: `Namaste! Your KrishiMitra password reset code is ${generatedOtp}. Do not share this with anyone. Code expires in 5 minutes.`,
      from: process.env.TWILIO_PHONE_NUMBER,
      to: `+91${phone}`
    });

    console.log(`\n🚜 --- KRISHIMITRA DEV MODE [RESET] ---`);
