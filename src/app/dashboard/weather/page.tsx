...
export default function WeatherIntelligence() {
  ...
  const [activeLocation, setActiveLocation] = useState<GeoLocation>({ name: 'Locating...', lat: 28.9845, lon: 77.7064 });
  ...
  // 1. Initial Load: Get User Location
  useEffect(() => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => initLocation(28.9845, 77.7064) // Fallback location
      );
    } else {
  ...
  // 2. Fetch Weather whenever Active Location changes
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      try {
        ...
        setCurrentWeather({
          ...
          condition: getWeatherDetails(current.weather_code).text,
        ...
        const forecastArray = daily.time.map((time: string, index: number) => {
          ...
          return {
            ...
            condition: getWeatherDetails(daily.weather_code[index]).text,
            icon: getWeatherDetails(daily.weather_code[index]).icon,
          };
        });
        setForecast(forecastArray);

        // Dynamic Agronomic Logic Engine
        ...
        if (daily.precipitation_probability_max[1] > 60 || daily.precipitation_probability_max[2] > 60) {
          newRecs.push({
            ...
            desc: `High probability of rain expected in the next 48 hours. Postpone scheduled irrigation to prevent waterlogging and nutrient leaching.`,
          ...
          newRecs.push({
            ...
            desc: 'No significant rain expected soon. Safe to proceed with standard watering cycles to maintain soil moisture.',
        ...
        if (current.wind_speed_10m > 18) {
          newRecs.push({
            ...
            desc: `Current wind speeds (${Math.round(current.wind_speed_10m)} km/h) are too high. Spraying pesticides now will result in severe chemical drift.`,
          ...
          newRecs.push({
            ...
            desc: `Low wind and low rain risk today makes it an ideal window for applying foliar fertilizers or pesticides.`,
        ...
        if (current.relative_humidity_2m > 75 && current.temperature_2m > 22 && current.temperature_2m < 30) {
          newRecs.push({
            ...
            desc: `Warm temperatures combined with ${current.relative_humidity_2m}% humidity creates ideal conditions for powdery mildew and rust. Monitor foliage closely.`,
        ...
        console.error("Failed to fetch weather telemetry", error);
        setIsLoading(false);
      }
    };

    if (activeLocation.name !== 'Locating...') {
      fetchWeather();
    }
  }, [activeLocation]);
  ...
  if (isLoading && !currentWeather) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <h2 className="text-xl font-bold text-agri-900">Calibrating Weather Instruments...</h2>
        <p className="text-gray-500">Connecting to global meteorological satellites</p>
      </div>
    );
  }
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="max-w-7xl mx-auto">
      ...
      <motion.div variants={item} className="mb-8">
        ...
        <p className="text-gray-500 mt-2 font-medium max-w-2xl">
          Real-time hyper-local weather data powering intelligent agronomic action plans to protect your yield.
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between h-full">
            <div className="flex flex-col justify-between flex-1 pr-0 md:pr-8">
              ...
              <div className="text-sm font-medium text-blue-200 flex items-center">
                ...
                {isLoading ? 'Synchronizing...' : `Live reading at: ${currentWeather?.lastUpdated}`}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-8 md:mt-0 md:min-w-[280px]">
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4">
                <div className="flex items-center text-blue-200 mb-2">
                  <Droplets className="w-5 h-5 mr-2" /> <span className="text-xs font-bold uppercase tracking-wider">Humidity</span>
                </div>
              ...
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4">
                <div className="flex items-center text-blue-200 mb-2">
                  <Wind className="w-5 h-5 mr-2" /> <span className="text-xs font-bold uppercase tracking-wider">Wind</span>
                </div>
              ...
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4">
                <div className="flex items-center text-blue-200 mb-2">
                  <CloudRain className="w-5 h-5 mr-2" /> <span className="text-xs font-bold uppercase tracking-wider">Rain Risk</span>
                </div>
              ...
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4">
                <div className="flex items-center text-blue-200 mb-2">
                  <ThermometerSun className="w-5 h-5 mr-2" /> <span className="text-xs font-bold uppercase tracking-wider">UV Index</span>
                </div>
        ...
        {/* Dynamic AI Agronomy Recommendations Box */}
        <motion.div variants={item} className="bg-white rounded-3xl p-6 shadow-xl shadow-gray-200/40 border border-gray-100 flex flex-col h-full z-10">
          ...
          {isLoading ? (
             <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-3">
               ...
               <p className="font-medium text-sm">Recalculating agronomist models...</p>
             </div>
          ) : (
            <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
              {recommendations.length > 0 ? recommendations.map((rec, idx) => (
                ...
                <div className="flex flex-col items-center justify-center h-full text-gray-400 text-center space-y-2">
                  ...
                  <p className="font-medium text-sm">Conditions are currently stable. No critical action required.</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            {forecast.map((day, idx) => (
              <div key={idx} className="flex flex-col items-center p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:bg-agri-50 hover:border-agri-200 transition-all group cursor-pointer">
                <span className="text-sm font-bold text-gray-400 mb-1">{idx === 0 ? 'Today' : day.date}</span>
                ...
                <span className="text-xs font-medium text-gray-500 mt-2 text-center leading-tight">{day.condition}</span>
              </div>
            ))}
          </div>
        )}
