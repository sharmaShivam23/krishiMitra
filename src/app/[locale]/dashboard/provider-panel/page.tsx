...
export default function ProviderPanel() {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        cropSuitability: formData.cropSuitability.split(',').map(s => s.trim()).filter(Boolean),
        diseaseTreats: formData.diseaseTreats.split(',').map(s => s.trim()).filter(Boolean),
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/provider/products/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white p-12 rounded-3xl border border-gray-100 text-center shadow-sm">
            ...
            <button onClick={() => setIsAddingMode(true)} className="text-emerald-600 font-bold hover:underline">Add your first product</button>
          </div>
        ) : (
            ...
            {products.map((p: any) => (
              <motion.div key={p._id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100 group">
                ...
                <div className="p-5">
                  ...
                  <div className="text-sm font-medium text-gray-500 flex items-center mb-4"><MapPin className="w-3.5 h-3.5 mr-1" /> {p.location.district}, {p.location.state}</div>
