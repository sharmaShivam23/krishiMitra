...
export default function GovernmentSchemes() {
  ...
  const toggleAudio = (scheme: SchemeData) => {
    ...
    if (playingId === scheme._id) {
      ...
      window.speechSynthesis.cancel();
      const utterance = new SpeechSynthesisUtterance(`${scheme.name}. Benefits include: ${scheme.benefits}`);
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h3 className="text-xl font-black text-agri-900 leading-tight pr-2">{scheme.name || t('unnamed')}</h3>
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-full transition-colors flex-shrink-0 ${playingId === scheme._id ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-400 hover:bg-agri-50 hover:text-agri-600'}`}
                    >
