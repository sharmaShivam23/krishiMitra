...
export default function EquipmentExchange() {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 pb-20 font-sans selection:bg-emerald-400 selection:text-emerald-950 flex flex-col overflow-x-hidden">

      {/* ═══════════════ HERO HEADER ═══════════════ */}
      <div className="bg-emerald-950 relative overflow-hidden pt-12 pb-0 px-4 sm:px-6 lg:px-8 border-b-[6px] border-emerald-500 w-full">
        {/* Background texture */}
        ...
        <div className="relative z-10 max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6">
          <div className="w-full md:w-auto">
            ...
            <p className="text-emerald-100/80 mt-2 text-base sm:text-lg max-w-xl font-medium">
              {t('subtitle')}
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-20 w-full">
        <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-4 sm:p-5 flex flex-col mb-8 border border-gray-100 gap-4">
          <div className="flex w-full sm:w-max bg-gray-100 p-1.5 rounded-xl self-start">
            <button
              ...
              className={`flex-1 sm:flex-none flex items-center justify-center space-x-2 px-4 sm:px-6 py-2.5 rounded-lg font-bold text-sm transition-all ${
                activeTab === 'rent' 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
             ...
             <div className="relative w-full">
                ...
                <select 
                  ...
                  className="w-full pl-9 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 focus:ring-2 focus:ring-emerald-500/20 outline-none appearance-none cursor-pointer transition-all truncate"
                >
             ...
             <div className="relative w-full">
                ...
                <select 
                  ...
                  className="w-full pl-9 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 focus:ring-2 focus:ring-emerald-500/20 outline-none appearance-none cursor-pointer truncate transition-all"
                >
             ...
             <div className="relative w-full">
                ...
                <select 
                  ...
                  className="w-full pl-9 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 focus:ring-2 focus:ring-emerald-500/20 outline-none appearance-none cursor-pointer disabled:opacity-50 truncate transition-all"
                >
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center py-32 w-full">
              <Loader2 className="w-12 h-12 animate-spin text-emerald-500 mb-4" />
              <p className="text-gray-500 font-medium">{t('loading')}</p>
            </motion.div>
          ) : error ? (
            <motion.div key="error" className="bg-red-50 border border-red-100 rounded-2xl p-10 text-center flex flex-col items-center w-full">
              <AlertCircle className="w-12 h-12 text-red-400 mb-4" />
              <h3 className="text-lg font-bold text-red-900">{t('errorTitle')}</h3>
              <p className="text-red-700 text-sm mt-1">{error}</p>
            </motion.div>
          ) : listings.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white border border-gray-200 border-dashed rounded-[2rem] p-10 sm:p-16 text-center flex flex-col items-center w-full">
                ...
                >
                  ...
                  <div className="h-48 bg-slate-100 relative overflow-hidden flex items-center justify-center shrink-0">
                    ...
                    <div className="absolute top-4 left-4 max-w-[80%]">
                      {activeTab === 'rent' ? (
                        <span className="bg-white/90 backdrop-blur-sm text-gray-800 text-xs font-bold px-3 py-1.5 rounded-lg flex items-center shadow-sm truncate">
                          <ShieldCheck className="w-3.5 h-3.5 mr-1 text-emerald-500 shrink-0" /> <span className="truncate">{t('condition')} {listing.equipment.condition}</span>
                        </span>
                      ) : (
                        <span className="bg-blue-600/90 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center shadow-sm truncate">
                          <User className="w-3.5 h-3.5 mr-1 shrink-0" /> <span className="truncate">{t('operatorIncluded')}</span>
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-5 sm:p-6 flex-1 flex flex-col min-w-0">
                    ...
                    <p className="text-sm text-gray-500 font-medium mb-4 flex items-start">
                      <MapPin className="w-4 h-4 mr-1 text-gray-400 shrink-0 mt-0.5" />
                      <span className="line-clamp-1">{listing.location.district}, {listing.location.state}</span>
                    </p>

                    <div className="flex items-center mb-4 bg-gray-50/80 w-fit max-w-full px-3 py-1.5 rounded-lg border border-gray-100 overflow-hidden">
                      <div className="w-6 h-6 rounded-full bg-emerald-100 flex items-center justify-center mr-2 text-emerald-700 font-bold text-xs shrink-0">
                        {typeof listing.providerId === 'object' && listing.providerId?.name ? listing.providerId.name.charAt(0).toUpperCase() : <User className="w-3 h-3 shrink-0" />}
                      </div>
                    ...
                    <div className="space-y-2 mt-auto">
                      <div className="flex items-center text-sm w-full">
                        <span className="text-gray-400 font-medium w-24 shrink-0">{t('equipmentLabel')}</span>
                        <span className="text-gray-800 font-bold truncate">{listing.equipment.name}</span>
                      </div>
                      
                      {activeTab === 'service' && listing.serviceDetails && (
                        <div className="flex items-center text-sm w-full">
                          <span className="text-gray-400 font-medium w-24 shrink-0">{t('jobTypeLabel')}</span>
                    ...
                    {currentUser && listing.providerId && ((typeof listing.providerId === 'string' ? listing.providerId : listing.providerId?._id) === currentUser._id) && (
                      <div className="flex flex-wrap gap-2 sm:gap-3 mb-4">
                        <Link href={`/dashboard/Services/post?listingId=${listing._id}`} className="rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-700 px-4 py-2 text-sm font-semibold hover:bg-emerald-100 transition flex-1 text-center">
                          {t('edit')}
                        </Link>
                        <button
                          type="button"
                          ...
                          className="rounded-xl border border-red-200 bg-red-50 text-red-700 px-4 py-2 text-sm font-semibold hover:bg-red-100 transition disabled:opacity-60 flex-1 text-center whitespace-nowrap"
                        >
